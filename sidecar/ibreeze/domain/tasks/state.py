...
DEPARTMENT_TASK_TRANSITIONS: MappingProxyType[str, frozenset[str]] = MappingProxyType({
    ...
    "checking_resources": frozenset({
        "ready", "waiting_dependency", "waiting_resource",
        "waiting_permission", "cancelled", "failed",
    }),
    "ready": frozenset({
        "executing", "waiting_dependency", "waiting_resource",
        "waiting_permission", "cancelled", "failed",
    }),
    "executing": frozenset({
        "reviewing", "waiting_dependency", "waiting_resource",
        "waiting_permission", "cancelled", "failed",
    }),
...
COMPANY_TASK_TRANSITIONS: MappingProxyType[str, frozenset[str]] = MappingProxyType({
    ...
    "analyzing": frozenset({
        "awaiting_user_confirmation", "waiting_resource",
        "revision_requested", "cancelled", "failed",
    }),
    "awaiting_user_confirmation": frozenset({
        "approved", "revision_requested", "rejected", "cancelled", "failed",
    }),
    ...
    "dispatching": frozenset({
        "checking_resources", "waiting_dependency", "waiting_resource",
        "waiting_permission", "cancelled", "failed",
    }),
    "checking_resources": frozenset({
        "executing", "waiting_dependency", "waiting_resource",
    ...
    "executing": frozenset({
        "reviewing", "waiting_dependency", "waiting_resource",
        "waiting_permission", "paused", "cancelled", "failed",
    }),
...
EMPLOYEE_TASK_ALL_EDGES: frozenset[tuple[str, str]] = frozenset(
    (src, tgt)
    for src, targets in EMPLOYEE_TASK_TRANSITIONS.items()
    for tgt in targets
)

DEPARTMENT_TASK_ALL_EDGES: frozenset[tuple[str, str]] = frozenset(
    (src, tgt)
    for src, targets in DEPARTMENT_TASK_TRANSITIONS.items()
    for tgt in targets
)

COMPANY_TASK_ALL_EDGES: frozenset[tuple[str, str]] = frozenset(
    (src, tgt)
    for src, targets in COMPANY_TASK_TRANSITIONS.items()
    for tgt in targets
)
