...
class TestCheckCompanyIsolation:
    async def test_returns_true_when_row_matches(self, db: aiosqlite.Connection):
        ...
        await db.execute("CREATE TEMPORARY TABLE test_iso (id TEXT, company_id TEXT)")
        await db.execute("INSERT INTO test_iso VALUES (?, ?)", (row_id, company_id))
    ...
    async def test_returns_false_when_row_missing(self, db: aiosqlite.Connection):
        await db.execute("CREATE TEMPORARY TABLE test_iso3 (id TEXT, company_id TEXT)")
        assert await check_company_isolation(db, "nonexistent", "test_iso3", "nonexistent") is False

    async def test_returns_false_when_wrong_company(self, db: aiosqlite.Connection):
        company_id = str(uuid.uuid4())
        row_id = str(uuid.uuid4())
        await db.execute("CREATE TEMPORARY TABLE test_iso2 (id TEXT, company_id TEXT)")
        await db.execute("INSERT INTO test_iso2 VALUES (?, ?)", (row_id, company_id))
        assert await check_company_isolation(db, "wrong-company", "test_iso2", row_id) is False


@pytest.mark.asyncio
class TestLockOptimistic:
    async def test_lock_succeeds(self, db: aiosqlite.Connection):
        await db.execute("CREATE TEMPORARY TABLE test_lock (id TEXT, version INTEGER)")
        await db.execute("INSERT INTO test_lock VALUES (?, ?)", ("row1", 1))
    ...
    async def test_lock_fails_on_version_mismatch(self, db: aiosqlite.Connection):
        await db.execute("CREATE TEMPORARY TABLE test_lock2 (id TEXT, version INTEGER)")
        await db.execute("INSERT INTO test_lock2 VALUES (?, ?)", ("row1", 1))
        result = await lock_optimistic(db, "test_lock2", "row1", 5)
        assert result is False

    async def test_lock_with_company_id(self, db: aiosqlite.Connection):
        await db.execute("CREATE TEMPORARY TABLE test_lock3 (id TEXT, version INTEGER, company_id TEXT)")
        company_id = str(uuid.uuid4())
        await db.execute("INSERT INTO test_lock3 VALUES (?, ?, ?)", ("row1", 1, company_id))
        result = await lock_optimistic(db, "test_lock3", "row1", 1, company_id=company_id)
        assert isinstance(result, bool)

    async def test_lock_with_wrong_company_id(self, db: aiosqlite.Connection):
        await db.execute("CREATE TEMPORARY TABLE test_lock4 (id TEXT, version INTEGER, company_id TEXT)")
        await db.execute("INSERT INTO test_lock4 VALUES (?, ?, ?)", ("row1", 1, "c1"))
        result = await lock_optimistic(db, "test_lock4", "row1", 1, company_id="wrong")
        assert isinstance(result, bool)
...
class TestIdempotency:
    ...
    async def test_complete_with_response(self, db: aiosqlite.Connection):
        ...
        await complete_idempotency(db, "test.method", "key6", response_json='{"ok":true}')
    ...
    async def test_complete_with_error(self, db: aiosqlite.Connection):
        ...
        await complete_idempotency(db, "test.method", "key7", error_code="BAD_REQUEST")
