...
def test_hooks_conversations_exists():
    assert (DESKTOP_DIR / "src" / "hooks" / "useConversation.ts").exists()


def test_hooks_tasks_exists():
    assert (DESKTOP_DIR / "src" / "hooks" / "useTask.ts").exists()
...
def test_types_is_valid():
    ...
    assert "export interface Workspace" in content


def test_hooks_conversations_is_valid():
    content = (DESKTOP_DIR / "src" / "hooks" / "useConversation.ts").read_text()
    assert "export function useListConversations" in content
    assert "export function useListMessages" in content
    assert "createRpcRequest" in content, "useConversation must use Tauri IPC invoke"
...
def test_login_page_no_mock():
    ...
    assert "tauriClient" in content, "LoginPage must use Tauri IPC invoke"
...
def test_register_page_no_mock():
    ...
    assert "tauriClient" in content, "RegisterPage must use Tauri IPC invoke"
...
def test_hooks_use_company_uses_invoke():
    ...
    assert "createRpcRequest" in content, "useCompany must use Tauri IPC invoke"
...
def test_hooks_use_knowledge_uses_invoke():
    ...
    assert "createRpcRequest" in content, "useKnowledge must use Tauri IPC invoke"
...
def test_hooks_use_workspace_uses_invoke():
    ...
    assert "createRpcRequest" in content, "useWorkspace must use Tauri IPC invoke"
...
def test_hooks_use_orchestration_uses_invoke():
    ...
    assert "createRpcRequest" in content, "useOrchestration must use Tauri IPC invoke"
...
def test_hooks_use_agent_uses_invoke():
    ...
    assert "createRpcRequest" in content, "useAgent must use Tauri IPC invoke"
