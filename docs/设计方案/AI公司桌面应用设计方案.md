...
响应固定为 `{message_id,company_task_id,task_status,intake_mode,analysis_queued}`：普通输入和基于旧任务重新发起分别返回 `task_status='draft'`、`intake_mode='new_task'/'superseding_task'`；要求修改计划返回 `task_status='revision_requested'`、`intake_mode='plan_revision'`；事务提交并成功写入分析 Outbox 时 `analysis_queued=true`。分析 Run 由 Outbox 消费者异步创建，响应不得虚构尚不存在的 `run_id`；调用方通过任务事件或 `task.get` 获得后续 Run。
...
executing → reviewing ↔ fixing
reviewing → final_review → completed
final_review → fixing
```
...
peer_reviewing → changes_requested → ready → running
...
| 表 | 关键职责 |
|---|---|
| `schema_migrations` | 本地 Schema 版本、脚本哈希和故障恢复状态 |
| `local_profile` | 后台来源与 app_user_id 的本地隔离映射 |
| `companies` | 公司名称、介绍和版本 |
| `departments` | 部门职能、负责人和状态 |
| `company_revisions/department_revisions` | 公司与部门不可变配置版本 |
...
| `company_plan_versions` | 待确认计划版本、哈希和验证凭据 |
| `department_tasks` | 部门任务与依赖 |
| `department_task_dependencies` | 部门任务有向依赖边 |
| `employee_tasks` | 职员任务 |
...
| `checkpoints` | 可恢复检查点 |
| `domain_events/projection_offsets/rpc_idempotency` | 领域事实、投影水位和写请求幂等结果 |
| `runtime_queue/runtime_company_fairness/runtime_leases` | 公平调度、公司虚拟完成时间和运行租约 |
| `tool_executions/human_approvals/verification_results` | 工具副作用、人工审批和验证证据 |
| `artifacts` | 产物、哈希和路径 |
...
| `knowledge_items/embedding_generations/knowledge_access_logs` | 本地知识事实、索引代际和检索审计 |
| `backup_records` | 备份、恢复和保留状态 |
| `audit_logs` | 本地操作审计 |
| `local_preferences` | 经过类型约束的Profile本地设置 |
...
| `KEYCHAIN_BUNDLE_CORRUPT` | Keychain session bundle 缺字段、损坏或轮换后状态不确定 |
...
`run_purpose` 枚举与 H.11 一致。`company_plan/summary` 的 `department_task_id/employee_task_id` 为 null 且 `work_item_id=company_task_id`；`interactive_turn` 的两个任务 id 为 null 且 `work_item_id=conversation_id`；`task_execution/merge` 的两个任务 id 必填且 `work_item_id=employee_task_id`，其中 merge 对应的 EmployeeTask 必须是编排器创建的 Merge Task；`review` 的 `work_item_id=review_assignments.id`；`verification` 的 `work_item_id=artifacts.id`，表示待验证的不可变 Artifact；`repair` 的 `work_item_id=review_issues.id`。后三类的任务 id 可为 null，但若提供必须属于同一公司任务链。`availability_snapshot_id/execution_snapshot_id` 必须通过 H.6 的员工、公司、work item、配置哈希和有效期一致性断言。`conversation_id` 必须属于同公司，后台知识索引作业不创建 AgentRunSpec。
...
1. 解析当前登录 Profile。
2. 按 F.9 得到唯一 `${profile_root}=profiles/{profile_directory_id}`，创建 `${profile_root}/run/{launch_uuid}`，权限 `0700`。
3. 生成 32 字节 CSPRNG 启动令牌。
4. 创建 UDS 路径 `${run_dir}/sidecar.sock`。
5. 以参数数组启动 Sidecar：`ibreeze-sidecar --socket {uds_path} --profile {profile_root} --app-version {version} --protocol-version {version}`。
...
握手只有在 Sidecar 已打开 `profile.db`、完成全部迁移、校验 `local_profile` 与启动身份一致且写队列可接收命令后才成功，响应固定为 `{ipc_session_id,protocol_version,profile_status:'ready',database_status:'ready',migration_version}`。Sidecar可先建立UDS并让该单一handshake请求等待初始化，但不得提前返回成功；Rust从发送handshake起最多等待10分钟并保持登录页的“正在准备本地数据”状态，超时后关闭连接、终止Sidecar进程组并返回安全诊断错误。迁移中断按H.1重跑。初始化失败返回稳定错误并退出Sidecar。实施阶段可用fake-sidecar验证Rust协议，但生产 `profile_opened` 只以真实Sidecar的该响应为准。
...
macOS Keychain service 固定为 `com.ibreeze.desktop`。账号名固定为 `{profile_directory_id}/session-bundle` 和 `{profile_directory_id}/provider/{credential_ref}`。前者的 value 是 `{schema_version,refresh_token,offline_session_ticket,family_id,issued_at}` JSON；Rust 先从 Keychain 读取并验证旧 value（不存在是首次保存），旧值已损坏时立即返回 `KEYCHAIN_BUNDLE_CORRUPT`。再在内存验证全部新字段，通过一次 `SecItemUpdate` 或首次 `SecItemAdd` 替换整个 value。调用失败时必须重新从 Keychain 读取：读回值等于旧 value 或仍不存在则确认未写入并返回原错误；等于新 value 则按“写入成功但响应丢失”继续；出现第三种值、JSON 损坏或必填字段缺失则返回 `KEYCHAIN_BUNDLE_CORRUPT`、禁止打开 Profile。若新bundle来自本次在线登录/Refresh且最终未确认保存成功，Rust必须用仍在内存的Access Token最佳努力撤销新family，再zeroize全部新凭据；撤销失败不改变本地判定，由服务端过期策略兜底。登录页此时只提供“清除本地会话并重新登录”：对本次尝试的 `profile_directory_id` 调用 `auth.logout` 删除损坏bundle，返回 `revoked_family=false`，保留Profile业务数据且不声称已远程撤销。比较完成后立即zeroize旧、新明文；不得用内存副本冒充故障后的读回结果。数据库只保存Provider凭据的随机UUID `credential_ref`，不得保存Keychain persistent reference原始字节、Token或API Key。删除Provider凭据时先删除Keychain item，再在本地事务把引用置unavailable；Keychain删除失败则数据库不变。
...
DomainEvent 类型注册表固定存放 `packages/contracts/domain-events/registry.v1.json`，v1 必须包含 `company.created/updated/archived`、`department.created/updated/leader_changed/archived`、`employee.created/updated/transferred/status_changed`、`conversation.user_message_submitted`、`task.created/plan_generated/plan_confirmed/status_changed/superseded`、`department_task.created/status_changed/report_submitted`、`employee_task.created/status_changed`、`artifact.created`、`review.assigned/submitted/issue_changed`、`approval.requested/resolved`、`workspace.status_changed` 和 `knowledge.imported/removed`。每个类型指向独立的 `packages/contracts/domain-events/{event_type}.v1.schema.json`，event type中的点保留；未注册类型、悬空Schema引用或多余Schema均使契约门禁失败。所有 payload 至少含 `company_id/aggregate_id/aggregate_version`，引用其他对象时必须含其 id，不嵌入 Prompt、Token 或完整文件正文。
...
2. 在该 `BEGIN IMMEDIATE` 事务内设置 `PRAGMA defer_foreign_keys=ON`；其他事务禁止设置。
...
9. Commit；SQLite 自动恢复 `defer_foreign_keys=OFF`，Command Handler 随即读取并断言为 `0`，回滚路径执行相同断言。
...
部门创建同理，固定预生成 Department、Revision、Leader Employee 和 Conversation id，并且只在该 `BEGIN IMMEDIATE` 事务内启用 deferred FK，提交或回滚后断言恢复为 `OFF`。任何失败均不得返回部分对象。
...
| `submitted` | `peer_reviewing/failed` |
...
| HumanApproval | `pending` | `allowed/denied/expired` |
| HumanApproval | `allowed` | `consumed/expired` |
| HumanApproval | `denied/expired/consumed` | — |
...
创建 AgentRun 时，Application Service 必须把 E.8 的 `department_task_id` 与 `work_item_id` 原样写入可索引列，并再次校验 purpose 映射：review 查询同公司 ReviewAssignment，verification 查询同公司待验证 Artifact，repair 查询同公司 ReviewIssue，merge 查询同公司且任务类型为 Merge Task 的 EmployeeTask；所查对象还必须与 ExecutionSnapshot 的 owner/work item 一致。外部写 ToolExecution 必须引用同 Run、类型为 `external_write`、目标哈希一致且状态为 `allowed` 的 `approval_id`；其他工具的 `approval_id` 必须为 null。Rust 返回目标、动作和结果哈希都匹配的 receipt 后，Sidecar 才能在同一事务把 ToolExecution 置为终态并把审批从 `allowed` 消费为 `consumed`。
...
```text
profiles/{profile_directory_id}/agent-state/codex/
profiles/{profile_directory_id}/agent-state/claude/
profiles/{profile_directory_id}/agent-state/opencode/config/
profiles/{profile_directory_id}/agent-state/opencode/data/
profiles/{profile_directory_id}/agent-state/opencode/cache/
```
...
无冲突时运行合并质量门禁后提交 merge commit。有冲突立即 `git merge --abort`，保存冲突路径和 base/ours/theirs blob SHA，创建 `task_kind=merge` 的 Merge EmployeeTask；对应 AgentRun 固定 `run_purpose=merge` 且 `work_item_id=employee_task_id`。Merge Task 只在 integration Worktree 执行。最终 integration branch 不自动合入用户 branch。
...
权限先过滤：company、department、task、private。`company` 对该公司 Run 可见；`department` 仅对 `department_id` 相同的职员 Run 可见；`task` 仅对当前 `company_task_id=task_id` 的 Run 可见；`private` 仅对 `employee_id=owner_employee_id` 的 Run 可见，即使同时记录 department/task 归属也不得扩大访问范围。当前 Run 的允许集合先由 SQLite 组合外键和上述谓词求出，再作为 FTS5 与 LanceDB 的候选白名单；任何检索实现都不能先全库召回再由模型过滤。用户本人可在 UI 查看全部本地公司数据，但 Agent 只按职员上下文授权。

算法：先读取公司的 active embedding generation；FTS5 和 LanceDB 查询都必须同时限定该 `generation_id` 与前述权限候选白名单。两路分别取 BM25 前 50、cosine 前 50，RRF `score += 1/(60+rank)`、重叠去重、前 12 进入 Context。每次检索必须写 `knowledge_access_logs`，记录 query SHA-256、权限范围、候选顺序、最终命中顺序和 Context Pack SHA-256；不得保存原始 query 文本。

只有 Review 通过 Artifact、完成报告、用户导入文件和用户显式标记消息可进入长期知识。token delta、工具日志、失败草稿和未通过 Review 产物禁止索引。

### J.8 SQLite 与 LanceDB 对账

`knowledge.import` 对每个新chunk写一条 `knowledge.imported`：`aggregate_type='knowledge_item'`、`aggregate_id=knowledge_items.id`、`aggregate_version=1`；`knowledge.remove` 对每个待删除行先以其 `version+1` 写 `knowledge.removed`，再删除该行。知识事实、这些DomainEvent和至少一个 `topic='knowledge.index.requested'` 的Outbox必须在同一事务提交；payload只含item/source/scope/hash标识，不含知识正文。Outbox消费者不创建AgentRun，而是向H.10提交 `work_item_type='knowledge_index'`、`work_item_id=company_id`、`run_id=null`、优先级30的内部任务；同一公司只能有一个active索引任务，后续事件合并为一次重建。

Worker 开始时读取该公司最新知识变更 `domain_events.row_sequence` 作为 `source_event_sequence`；从未产生知识变更的空公司使用0。随后在同一个 SQLite 读快照取得全部当前 knowledge id、内容哈希和正文。Embedding 在 ProcessPool 计算，结果最多累计 128 条便向容量 32 的单写队列提交一个批次事务，生产者在队列满时等待，禁止逐条写入或绕过写队列。LanceDB 和 FTS5 都写入新的 building generation，不改当前 active generation。

构建完成后重新读取当前知识 id/哈希集合与最新知识变更序号；只有二者都等于构建快照且向量维度为 384 时，才在一个写事务内把所有当前 `knowledge_items.embedding_generation_id` 更新为新 generation、新 generation 设 active、旧 generation 设 retired。FTS行与Lance向量都带generation id，因此切换前后查询始终只读取一个完整active generation；旧generation数据在切换提交后由可重试清理器删除，清理失败不影响正确性。期间出现新导入或删除则把本 generation 设 failed并清理其索引数据，再合并排队最新重建，旧active继续服务。

启动和每次索引任务完成后都按公司对账：无 active generation、存在 `embedding_generation_id IS NULL`、active generation 的 `source_event_sequence` 小于最新知识变更序号，或 active generation 的 id/哈希集合与 SQLite 不一致，任一成立即排队全量重建。building 超过 1 小时标记 failed 后同样排队。空知识集合也必须生成可激活的空 generation，从而使“删除最后一条知识”可被水位证明，不能只依靠扫描 null 行发现变化。
...
`auth.login` 固定执行 REST 登录 → 验证响应与OfflineSessionTicket → 单次Keychain bundle update与故障后读回确认 → 获取并验签Catalog → 原子更新Profile meta → 启动Sidecar → Sidecar打开数据库、完成迁移并校验 `local_profile` → 首次 health 为 healthy → 返回 `profile_opened`。任一步失败不得返回已打开状态；Keychain更新后的后续失败保留有效bundle，关闭Sidecar并回登录页。`auth.openProfile` 在后台可达时必须先Refresh并按相同流程轮换bundle、同步Catalog；仅网络失败时才验证本地OfflineSessionTicket并以offline打开。`auth.changePassword` 只接受受限改密session，成功响应按G.11轮换后继续执行登录后半段。`auth.logout` 在离线模式无法联系后台时关闭本地Profile并删除bundle，后端family依靠Refresh过期/服务端撤销策略失效；UI必须明确显示离线退出未完成远端撤销。若最近一次打开尝试因 `KEYCHAIN_BUNDLE_CORRUPT` 失败，Rust保留的仅是该次非敏感 `profile_directory_id`；signed-out状态下的 `auth.logout` 只允许删除这个损坏item并返回 `{closed_profile:false,revoked_family:false}`，随后清除该id，不打开或删除Profile数据。

`conversation.submitUserMessage.response.schema.json` 必须把 10.3 的响应落实为封闭对象：`message_id/company_task_id` 为 UUID，`task_status` 只允许 `draft/revision_requested`，`intake_mode` 只允许 `new_task/plan_revision/superseding_task`，`analysis_queued` 只允许 true，且不得包含 `run_id`。

| 方法 | 类型 | 幂等保留 | 主要错误 |
|---|---|---|---|
| `backend.validateOrigin` | 写 | 仅in-flight | `VALIDATION_FAILED` |
| `auth.register` | 写 | 仅in-flight | `AUTH_EMAIL_EXISTS` |
| `auth.login` | 写 | 仅in-flight | `AUTH_INVALID_CREDENTIALS`、`KEYCHAIN_BUNDLE_CORRUPT` |
| `auth.changePassword` | 写 | 仅in-flight | `AUTH_PASSWORD_CHANGE_REQUIRED`、`KEYCHAIN_BUNDLE_CORRUPT` |
...
手写可执行源码按语言原生可测指标全部达到 100%：TypeScript/TSX 的 statements、branches、functions、lines；Python 的 statements/lines 与 branches；Rust 的 lines、functions、regions。仅排除自动生成 DTO、纯 DDL、无逻辑启动入口和签名公证外部包装；排除写入 `coverage-exclusions.yml`。禁止以 pragma 排除普通业务分支。

Seatbelt模板与转义可在普通CI使用fake执行器验证，但发布门禁必须在最低和最高受支持macOS版本的真实机器执行I.8的最小探测与逃逸用例；该门禁不可降为nightly或人工抽查。任一版本缺少可执行 `sandbox-exec`、SBPL行为不符或没有对应真实runner时禁止发布该桌面版本，并把CLI职员标为unavailable；API Model职员不依赖Seatbelt，仍可使用Built-in Runtime。
