...
async fn supervisor_initial_state() {
    ...
    assert!(!supervisor.is_running().await, "supervisor must start not running");
...
async fn stop_empty_supervisor_returns_false() {
    let supervisor = SidecarSupervisor::new();
    let result = supervisor.stop().await.expect("stop empty supervisor must succeed");
...
async fn restart_tracker_tracks_restarts() {
    ...
    supervisor.record_restart().await;
    supervisor.record_restart().await;
    assert!(supervisor.is_throttled().await, "throttled after 3 restarts");
}
