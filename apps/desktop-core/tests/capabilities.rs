...
fn capabilities_restrict_commands() {
    ...
    let content =
        std::fs::read_to_string(&cap_path).expect("capabilities/main.json must be readable");
    let value: serde_json::Value =
        serde_json::from_str(&content).expect("capabilities/main.json must be valid JSON");
    ...
    assert!(
        ...
        "capabilities must define permissions"
    );

    let all_perms: Vec<&str> = permissions.iter().filter_map(|p| p.as_str()).collect();
    ...
    assert_eq!(
        value["identifier"], "main-capability",
...
fn csp_restricts_sources() {
    ...
    let content = std::fs::read_to_string(&conf_path).expect("tauri.conf.json must be readable");
    let value: serde_json::Value =
    ...
    assert!(
        !csp.contains("frame-src *"),
        "CSP must not allow frame-src *"
    );
    assert!(
        csp.contains("form-action 'none'"),
        "CSP must disable form-action"
    );
}
